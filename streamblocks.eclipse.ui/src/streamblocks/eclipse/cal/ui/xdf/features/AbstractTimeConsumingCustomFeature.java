 ...
 * 
 */
public abstract class AbstractTimeConsumingCustomFeature extends AbstractCustomFeature {
	 ...
	 * create tasks (and eventually sub-tasks), notify for worked and done tasks and
	 * check if user cancelled the task.
	 * 
	 * @param context The CustomFeature context
	 * @param monitor The monitor used to manage progress bar and Job cancellation
	...
	/**
	 * Callback launched immediately after job execution in the Job Thread. Default
	 ...
	 * Initialize the Job.
	 * 
	 * @param context The CustomContext that will be given to
	...
	protected Job initializeJob(final ICustomContext context) {
		return new Job(getName()) {
			...
			protected IStatus run(final IProgressMonitor monitor) {
				...
				final TransactionalEditingDomain editDomain = TransactionUtil.getEditingDomain(getDiagram());

				final RecordingCommand command = new RecordingCommand(editDomain, getName()) {
					...
					protected void doExecute() {
						try {
							AbstractTimeConsumingCustomFeature.this.execute(context, monitor);
	 ...
	 * 
	 * @param job The Job instance to configure
	 */

...
import streamblocks.eclipse.cal.ui.xdf.patterns.NetworkPortPattern;
...
public class XdfDiagramLayoutManager {
	...
	XdfDiagramLayoutManager(final Diagram diagram) {
		...
		for (final Shape shape : diagram.getChildren()) {
			...
			} else if (eObject instanceof AstPort) {
				registerPort(shape);
			} else {
				// ??
			}
	...
	private void applyLayoutToConnection(final PictogramElement pe, final KEdge edge) {
		...
		if (!(pe instanceof FreeFormConnection)) {
			throw new RuntimeException(pe.getClass() + " connection type is not supported.");
		} else if (!(ga instanceof Polyline)) {
			throw new RuntimeException(ga.getClass() + " connection graphics type is not supported.");
		}

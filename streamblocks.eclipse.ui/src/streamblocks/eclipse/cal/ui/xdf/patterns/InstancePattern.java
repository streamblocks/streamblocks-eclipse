...
public class InstancePattern extends AbstractPattern {
	...
	public boolean isMainBusinessObjectApplicable(Object object) {
		if (object instanceof AstPort) {
			final AstPort port = (AstPort) object;
			final EObject ctr = port.eContainer();
			if (ctr instanceof AstActor) {
				return true;
			} else if (ctr instanceof AstExternalActor) {
				return true;
			} else if (ctr instanceof AstNetwork) {
		...
		return object instanceof AstEntity;
	}
	...
	public String getInitialValue(IDirectEditingContext context) {
		final AstEntityVariable obj = (AstEntityVariable) getBusinessObjectForPictogramElement(
	...
	public void setValue(String value, IDirectEditingContext context) {
		...
		final AstEntityVariable obj = (AstEntityVariable) getBusinessObjectForPictogramElement(pe);
		obj.setName(value);
	...
	public String checkValueValid(String value, IDirectEditingContext context) {
		final AstEntityVariable instance = (AstEntityVariable) getBusinessObjectForPictogramElement(
				context.getPictogramElement());
		return checkValueValid(value, instance);
	}

	public String checkValueValid(final String value, final AstEntityVariable instance) {
		if (value.length() < 1) {
		...
		for (final AstPort inputPort : network.getInputs()) {
			if (instance.getName().equals(inputPort.getName())) {
				return "The network already contains an instance with the same name (" + instance.getName() + ")";
			}
		}

		for (final AstEntityVariable entity : network.getEntities()) {
			if (entity != instance && instance.getName().equals(entity.getName())) {
				return "The network already contains an entity with the same name (" + instance.getName() + ")";
			}
		}

		for (final AstPort outputPort : network.getOutputs()) {
			if (instance.getName().equals(outputPort.getName())) {
				return "The network already contains an entity with the same name (" + instance.getName() + ")";
			}
		}
	...
	public void preDelete(IDeleteContext context) {
		...
		if (pe instanceof AnchorContainer) {
			XdfUtil.deleteConnections(getFeatureProvider(), (AnchorContainer) pe);
		}
	}
	...
	public boolean canCreate(ICreateContext context) {
		...
		if (context.getTargetContainer() instanceof Diagram) {
			...
			if (getBusinessObjectForPictogramElement(context.getTargetContainer()) instanceof AstNetwork) {
				return true;
			}
		}
		return false;
	...
	public Object[] create(ICreateContext context) {
		...
		final AstEntityVariable newInstance = CalFactory.eINSTANCE.createAstEntityVariable();
	...
	public PictogramElement add(IAddContext context) {
		...
		final AstEntityVariable addedDomainObject = (AstEntityVariable) context.getNewObject();

		// Add the new Instance to the current Network
		final AstNetwork network = (AstNetwork) getBusinessObjectForPictogramElement(getDiagram());
		network.getEntities().add(addedDomainObject);
	...
	public boolean canMoveShape(IMoveShapeContext context) {
		return isPatternRoot(context.getPictogramElement()) && context.getTargetContainer() == getDiagram();
	}
	...
	public IReason updateNeeded(IUpdateContext context) {
		...
		if (!isPatternRoot(pe)) {
			return Reason.createFalseReason("Given PE is not an Instance shape");
		}
		...
		final AstEntityVariable instance = (AstEntityVariable) getBusinessObjectForPictogramElement(pe);
		...
		// Initialize the reason object, used in following tests
		final IReason portsUpdatedReason = Reason.createTrueReason("The port order or their names have to be updated.");
		//final AstEntity entity = instance.getAdapter(AstEntity.class);

		final AstAbstractActor entity = ((AstEntityInstanceExpr) instance.getEntity()).getType().getActor();
				
		if (inNames.size() != entity.getInputs().size() || outNames.size() != entity.getOutputs().size()) {
	...
	public boolean update(IUpdateContext context) {
		...
		if (PropsUtil.isInstance(pe)) {
			...
			updateRefinementAndRestoreConnections(instanceShape, instance.getEntity(),
	...
	/**
	 * <p>
	 * Update the refinement (Instance or Network) for the instance linked to the
	 * given instanceShape. The input and output ports of the given entity are added
	 * to the shape.
	 * </p>
	 * 
	 * <p>
	 ...
	 * </p>
	 * 
	 * @param instanceShape The instance to refine
	 * @param refinement    The entity to refine the instance on
	 * @return true if the refinement has been performed
	 */
	private boolean updateRefinement(final ContainerShape instanceShape, final EObject refinement) {
		Assert.isNotNull(refinement, "Given Entity must not be null");
		if (!isPatternRoot(instanceShape)) {
			return false;
		}
		if (!(refinement instanceof AstEntity)) {
			return false;
		}
		...
		instanceExpr.setType((AstEntity) refinement);
		...
		final List<Anchor> anchors = new ArrayList<Anchor>(instanceShape.getAnchors());
		for (final Anchor anchor : anchors) {
			EcoreUtil.delete(anchor, true);
		}

		// Add ports
		AstAbstractActor abstractActor = ((AstEntity) refinement).getActor();

		if (abstractActor instanceof AstActor || abstractActor instanceof AstExternalActor) {
			addPorts(instanceShape, abstractActor.getInputs(), Direction.INPUTS);
			addPorts(instanceShape, abstractActor.getOutputs(), Direction.OUTPUTS);
	 ...
	 * {@link #updateRefinement(ContainerShape, EObject)} but in addition, it saves
	 * existing connections from/to the instance and try to restore them after
	 * performing the refinement update. Restoration is done based on ports name for
	 * connections source/target.
	 * </p>
	 * 
	 * <p>
	 * This method must not be called with 'null' as given refinement. In that case,
	 * {@link #deleteRefinement(ContainerShape)} must be used instead
	 * </p>
	 * 
	 * @param instanceShape The instance shape to refine
	 * @param refinement    The new actor/network to refine this instance on (Must
	 *                      not be null)
	 * @param msg           The beginning of the message displayed to user at the
	 *                      end of the process
	 * @return true if the refinement has been performed
	 */
	public boolean updateRefinementAndRestoreConnections(final ContainerShape instanceShape, final EObject refinement,
			final String msg) {
		Assert.isNotNull(refinement, "Given Entity must not be null");
		...
		for (final Anchor anchor : instanceShape.getAnchors()) {
			...
			if (anchor.getIncomingConnections().size() >= 1) {
				...
				final List<Connection> conList = new ArrayList<Connection>(anchor.getOutgoingConnections());
...
//		final AstAbstractActor entity = ((AstEntityInstanceExpr) ((AstEntityVariable) getBusinessObjectForPictogramElement(
//				instanceShape)).getEntity()).getType().getActor();
		...
		for (final Anchor anchor : instanceShape.getAnchors()) {
			...
			if (PropsUtil.isInstanceInPort(anchor) && incomingMap.containsKey(portName)) {
				...
				dfConnection.setOutPort(portName);
				
				// Update Graphiti connection
				connection.setEnd(anchor);
				cptReconnectedTo++;
			} else if (PropsUtil.isInstanceOutPort(anchor) && outgoingMap.containsKey(portName)) {
				...
				for (final Connection connection : outgoingMap.remove(portName)) {
					...
					dfConnection.setInPort(portName);
		...
		if (cptReconnectedTo > 0) {
			infoMsg.append(cptReconnectedTo).append(" connection(s) reconnected to input port(s).").append('\n');
		}
		if (cptReconnectedFrom > 0) {
			infoMsg.append(cptReconnectedFrom).append(" connection(s) reconnected from output port(s).").append('\n');
		}
		if (cptDeletedConnections > 0) {
			infoMsg.append(cptDeletedConnections).append(" connection(s) deleted from the network.").append('\n');
		}

		// Inform the user about what happened
		MessageDialog.openInformation(XdfUtil.getDefaultShell(), "Instance update finished", infoMsg.toString());
	 ...
	 * business object. This method remove all ports from the shape and update its
	 * properties, style and size.
	 * 
	 * @param instanceShape
	...
	public boolean deleteRefinement(final ContainerShape instanceShape) {
		...
		instanceShape.getGraphicsAlgorithm().setStyle(StyleUtil.basicInstanceShape(getDiagram()));
	 ...
	 * layouting (setup of sizes and location) of each element is done in the layout
	 * method. layout() is called explicitly or implicitly from some methods.
	 * 
	 * {@link #updateRefinement(ContainerShape, EObject)} calls this method and
	 * apply a resize just after. The layout() method is called from resizeShape()
	 * method.
	 * 
	 * @param instanceShape The instance shape
	 * @param ports         The list of ports
	 * @param direction     The type of ports (inputs or outputs)
	 */
	private void addPorts(final ContainerShape instanceShape, final List<AstPort> ports, final Direction direction) {
	 ...
	 * 
	 * @param pe The instance pictogram element
	 */
	...
	/**
	 * Calculate the minimal height needed to display the longest port list (between
	 * inputs and outputs ones)
	 * 
	 * @param pe The instance pictogram element
	...
	private int getInstanceMinHeight(final PictogramElement pe) {
		...
		for (final GraphicsAlgorithm child : instanceShape.getGraphicsAlgorithm().getGraphicsAlgorithmChildren()) {
			if (child instanceof Text && (PropsUtil.isInstanceInPort(child) || PropsUtil.isInstanceOutPort(child))) {
		...
		if (portLineHeight != 0) {
			final int totalMinHeight = LABEL_HEIGHT + SEPARATOR + PORT_MARGIN
					+ nbMaxPorts * (portLineHeight + PORT_MARGIN) + PORT_MARGIN;
	 ...
	 * @return
	 */
	public Anchor getAnchorForPort(final PictogramElement instancePe, final AstPort port) {
	...
	// FIXME
	public Anchor getAnchorForPort(final PictogramElement instancePe, String port) {
		if (isPatternRoot(instancePe)) {
			final ContainerShape instanceShape = (ContainerShape) instancePe;
			for (final Anchor anchor : instanceShape.getAnchors()) {
				if (getBusinessObjectForPictogramElement(anchor).equals(port)) {
	...
	/**
	 * Returns the Text object used to display the port name. The object is searched
	 * from the given anchor, which must represents an instance port.
	 * 
	 * @param anchor
	 * @return
	 */
	public Text getTextFromAnchor(final Anchor anchor) {
		...
		for (final GraphicsAlgorithm gaChild : anchor.getParent().getGraphicsAlgorithm()
